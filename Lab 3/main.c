...
int app_thread_throughput(void *arg)
{
	...
	printf("Hello from slave core %u, and I going to receive echoed packets!\n", lcore_id);
		...
		for( ; ; ) {
			...
			while(rte_get_tsc_cycles() - p_start < p_ticks)
			{
				...
				for(i=0;i<n_pkts;i++)
				rte_ring_enqueue(ring,pkts[i]);	
				total_pkts += n_pkts;
				for(i=0; i<n_pkts; i++) {
					rte_pktmbuf_free(pkts[i]);
				}
			}
			printf("lcore %u: Received %u echoed packets in last %d seconds\n", lcore_id, total_pkts, total_time_in_sec);
		}
			}
	else {

		
		struct rte_mbuf *pkts_tx[RTE_PORT_IN_BURST_SIZE_MAX];
		uint32_t pmd_id;
		int cnt = 64;
		int rte_ring_dequeue_bulk(ring,pkts_tx,cnt);	
		

		//rte_eth_tx_burst(pmd_id, lcoreid_to_queueid[pmd_id][lcore_id], &pkts_tx, cnt);
		for (pmd_id = 0; pmd_id < RTE_MAX_ETHPORTS; pmd_id++)
		{
...
int main(int argc, char **argv)
{
	srand(time(NULL));
	app_init(argc, argv);
	char name_ring[128]; sprintf(name_ring, "Ring%u.0");
	ring=rte_ring_create(name_ring,RTE_PORT_IN_BURST_SIZE_MAX,0,RING_F_SP_ENQ|RING_F_SC_DEQ);
