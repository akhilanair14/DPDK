...
uint32_t heavy_hitter_ip_src, heavy_hitter_ip_dst;
...
int app_thread(void *arg)
{
	...
	if(lcore_id == master_core_id)
	{
		...
		{
		uint32_t hash;
						   hash=((flow.ip_src.s_addr)+(flow.ip_dst.s_addr)*(flow.sourcePort)-(flow.destPort)-(flow.protocol));				
	...
	hash=hash%6;
		...
		uint32_t hv[3];
	     ...
	     hv[2] = hash2(flow);                    									
				...
				printf("\n Source IP address is  %s",inet_ntoa(heavy_hitter.flow.ip_src));
				printf("\n Dest IP address is  %s",inet_ntoa(heavy_hitter.flow.ip_dst));
				printf("\n source port is %d", ntohs(heavy_hitter.flow.sourcePort));
				printf("\n dest port is %d", ntohs(heavy_hitter.flow.destPort));
				printf("\n protocol is %d",heavy_hitter.flow.protocol);
